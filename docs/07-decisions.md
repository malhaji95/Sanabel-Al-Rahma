...
| 2026-09-02 | T-03 | "Excel import" — reading `.xlsx` directly needs a new package, and CLAUDE.md §3 says not to add one without asking. | Import reads **CSV**, which is what Excel writes with "Save as CSV". Swapping in a real `.xlsx` reader later is one class. | **Resolved** — approved 2026-09-07. A *Download template* button now sits beside *Import* on both reference screens; the file carries the columns, a UTF-8 byte-order mark and every region name already spelled, so the admin only fills in the amount column and hands it back. |
